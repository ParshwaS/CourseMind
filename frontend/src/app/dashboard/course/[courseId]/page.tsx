...
export default function CoursePage() {
  ...
  useEffect(() => {
    const _ = Array.isArray(courseId) ? courseId[0] : courseId;
    if (_) {
      modulesService.getByCourseId(_).then((data) => {
        setModules(data);
      });
    }
  }, [courseId]);
      ...
      <Dialog open={isNewModuleDialogOpen} onOpenChange={setIsNewModuleDialogOpen}>
        <DialogContent>
          ...
          <div className="grid w-full max-w-sm items-center gap-1.5">
            ...
            />
            <Button onClick={() => addModule(courseId)}>Create Module</Button>
          </div>
        </DialogContent>
      </Dialog>
